...
namespace DirectX
{
    namespace SimpleMath
    {
		...
		//------------------------------------------------------------------------------
		// convert 3D tranform matrix to 2d
		D2D1::Matrix3x2F XMMATRIXConvertToMatrix3x2F(const  DirectX::XMMATRIX& m);
		//------------------------------------------------------------------------------
		// 2D size
		struct Size
		{
			float width;
			float height;
			Size(float w = 0.f, float h = 0.f) noexcept :width(w), height(h){}
